...
const AuthView = ({ authForm, setAuthForm, onAuth, onGetStarted, showNotification }) => {
  const [loginData, setLoginData] = useState({ 
    email: '', 
    password: '', 
    sas_base_url: 'https://sas.jobdesk.com/api' 
  });
  const [resetEmail, setResetEmail] = useState('');
  const [otpValues, setOtpValues] = useState(['', '', '', '', '', '']);
  const [isLoading, setIsLoading] = useState(false);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            Jobdesk Spider Plugin
          </h1>
          <p className="text-gray-600">
            {authForm === 'login' && 'Sign in to manage your spiders'}
            {authForm === 'forgot' && 'Reset your password'}
            {authForm === 'verify' && 'Two-Factor Authentication'}
            {authForm === 'success' && 'Welcome back!'}
          </p>
        </div>
          ...
          <div className="p-8">
            ...
            {authForm === 'login' && (
              <div className="space-y-5">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all disabled:bg-gray-50 disabled:cursor-not-allowed"
                    />
                  </div>
                </div>
                ...
                <button
                  onClick={handleLogin}
                  disabled={isLoading}
                  className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-3 rounded-lg font-medium hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                    <>
            ...
            {authForm === 'forgot' && (
              <div className="space-y-5">
                ...
                <button
                  onClick={handleForgotPassword}
                  className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-3 rounded-lg font-medium hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all"
                >
                  Send Reset Link
                </button>
                ...
                <button
                  ...
                  className="w-full text-indigo-600 hover:text-indigo-700 font-medium transition-colors flex items-center justify-center gap-2"
                >
                  <ArrowLeft size={16} />
                  Back to Sign In
                </button>
                ...
                <div className="flex gap-2 justify-center" onPaste={handleOtpPaste}>
                  {otpValues.map((value, index) => (
                    <input
                      ...
                      disabled={isLoading}
                      className="w-12 h-12 text-center text-xl font-semibold border-2 border-gray-300 rounded-lg focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all disabled:bg-gray-50 disabled:cursor-not-allowed"
                    />
                  ))}
                </div>
            ...
            {authForm === 'success' && (
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 mb-6">
                  You can now use Jobdesk Spider features
                </p>
                {isExternal && (
                ...
                <button
                  onClick={onGetStarted}
                  className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-8 py-3 rounded-lg font-medium hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all"
                >
                  Get Started
                </button>
              </div>
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          © 2024 Jobdesk. All rights reserved.
        </p>
      </div>
    </div>
  );
